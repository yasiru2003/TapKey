...
import { canonicalizeCountPattern, canonicalizeShiftedPin } from "../src/shared/canonicalize.js";
...
describe("Factor 2 integration boundary", () => {
  it("returns a verification result for SB2 shifted PIN without creating sessions", async () => {
    const repository = new InMemorySpacebarSecretRepository();
    const tapCounts: [number, number, number, number] = [6, 1, 8, 3];
  ...
  it("returns a verification result for SB1 count pattern without creating sessions", async () => {
    const repository = new InMemorySpacebarSecretRepository();
    await repository.save({
      userId: "user-2",
      mode: "COUNT",
    ...
    await expect(verifyPattern("user-2", { mode: "COUNT", groups: [3, 1, 4, 2] }, repository))
      .resolves.toEqual({ success: true, userId: "user-2" });
  });
});